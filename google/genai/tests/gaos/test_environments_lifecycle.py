...
def test_python_environments_files_wrapper_error_handling():
  wrapper = GeminiNextGenEnvironmentFiles(parent=object())
  with pytest.raises(
      AttributeError,
      match="environments.files.list is not available on this client.",
  ):
    wrapper.list(environment="env_123", path="src/main.py")

  async_wrapper = AsyncGeminiNextGenEnvironmentFiles(parent=object())
  import asyncio
  ...
  with pytest.raises(
    ...
    asyncio.run(async_wrapper.list(environment="env_123", path="src/main.py"))

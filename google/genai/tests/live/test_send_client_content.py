...
async def test_send_text(mock_api_client, mock_websocket, vertexai):
  ...
  await session.send_client_content(turns='test')
...
async def test_send_file(mock_api_client, mock_websocket, vertexai):
  ...
  await session.send_client_content(turns=input)
  mock_websocket.send.assert_called_once()
  sent_data = json.loads(mock_websocket.send.call_args[0][0])
  assert 'client_content' in sent_data
...
@pytest.mark.parametrize('vertexai', [True, False])
@pytest.mark.asyncio
async def test_send_inline_data(mock_api_client, mock_websocket, vertexai):
  session = live.AsyncSession(
      api_client=mock_api_client(vertexai=vertexai), websocket=mock_websocket
  )
  input = types.Part(

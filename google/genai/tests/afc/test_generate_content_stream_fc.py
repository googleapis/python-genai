...
@pytest.fixture
def mock_generate_content_stream_with_fc():
  with mock.patch.object(
      models.Models, '_generate_content_stream'
...
@pytest.fixture
def mock_generate_content_stream_with_fc_async():
  with mock.patch.object(
      models.AsyncModels, '_generate_content_stream'
  ) as mock_stream_with_afc:
...
def test_generate_content_stream_with_thought_summaries(
    mock_generate_content_stream_with_fc,
):
  ...
  chunk = None
  for chunk in stream:
    assert chunk.function_calls == [TEST_FUNCTION_CALL_PART.function_call]

  assert mock_generate_content_stream_with_fc.call_count == 1
...
async def test_generate_content_stream_with_function_tools_used_async(
    mock_generate_content_stream_with_fc_async,
):
  ...
  assert mock_generate_content_stream_with_fc_async.call_count == 1

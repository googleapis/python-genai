...
from ._operations_converters import (
    ...
    _UploadToRagStoreOperation_from_mldev,
)
...
class AuthType(_common.CaseInSensitiveEnum):
  ...
  OAUTH = 'OAUTH'
  """OAuth auth."""
  CLIENT_CERTIFICATE_AUTH = 'CLIENT_CERTIFICATE_AUTH'
  """Client Certificate Mutual TLS (mTLS) Auth."""
  OIDC_AUTH = 'OIDC_AUTH'
...
class FilteredReason(_common.CaseInSensitiveEnum):
  """Output only. The reason for filtering out the example."""

  FILTERED_REASON_UNSPECIFIED = 'FILTERED_REASON_UNSPECIFIED'
  """No reason specified. Should not be used."""
  RECITATION = 'RECITATION'
  """The example is filtered out because of potential recitation."""
  SPII = 'SPII'
  """The example is filtered out because the content potentially contains Sensitive Personally Identifiable Information (SPII)."""
  PROHIBITED_CONTENT = 'PROHIBITED_CONTENT'
  """The example is filtered out for potentially containing prohibited content."""
  SAFETY = 'SAFETY'
  """The example is filtered out for potentially containing safety issues."""


class AiFeedbackTaskType(_common.CaseInSensitiveEnum):
  """Required.

  Type of task used to judge AI feedback. The best AI-generated candidate is
  chosen based on overall metrics for computed for a given task. Full criteria
  are described in the TaskType.
  """

  AI_FEEDBACK_TASK_TYPE_UNSPECIFIED = 'AI_FEEDBACK_TASK_TYPE_UNSPECIFIED'
  """No task specified. Should not be used."""
  SUMMARIZATION = 'SUMMARIZATION'
  """Judges summarization quality according to the following criteria: 1. Follows instructions - To what extent does the model's response demonstrate an understanding of the instruction from the prompt? 2. Grounded - Does the response include only information from the inference context and inference instruction? 3. Comprehensive - To what extent does the model capture key details in the summarization? 4. Brief - Is the summarization verbose? Does it include flowery language? Is it overly terse?"""
  QUESTION_ANSWERING = 'QUESTION_ANSWERING'
  """Judges question answering quality according to the following criteria: 1. Fully answers the question - The answer responds to the question, completely. 2. Grounded - Does the response include only information from the instruction context and inference instruction? 3. Relevance - Does the content of the answer relate to the question? 4. Comprehensive - To what extent does the model capture key details in the question?"""
...
class ResponseParseType(_common.CaseInSensitiveEnum):
  ...
  """Use regex to extract the important part of sample response. Same as [GoogleSQL](https://cloud.google.com/bigquery/docs/reference/standard-sql/string_functions#regexp_extract) `REGEX_EXTRACT(response, expression)`."""
...
class Blob(_common.BaseModel):
  ...
  tool_input_only: Optional[bool] = Field(
      default=None,
      description="""Optional. The flag is useful for sending large data to the tools as input without exceeding the model context window limit. Model won't have access to the blob data but will be aware that it can use the blob as tool input. This field is only supported when the request has code_execution, google_search, or url_context tool enabled.""",
  )
  ...
  will_continue: Optional[bool] = Field(
      default=None,
      description="""Optional. When set indicates that the next message in a streaming connection is to be interpreted as a continuation of this Blob. The next message must contain only the Content of the same role as the the one containing this Part, and must contain a single Part with inline_data and the same mime_type as this Blob.""",
  )
...
class BlobDict(TypedDict, total=False):
  ...
  tool_input_only: Optional[bool]
  """Optional. The flag is useful for sending large data to the tools as input without exceeding the model context window limit. Model won't have access to the blob data but will be aware that it can use the blob as tool input. This field is only supported when the request has code_execution, google_search, or url_context tool enabled."""
  ...
  """Optional. When set indicates that the next message in a streaming connection is to be interpreted as a continuation of this Blob. The next message must contain only the Content of the same role as the the one containing this Part, and must contain a single Part with inline_data and the same mime_type as this Blob."""
...
class PartialArg(_common.BaseModel):
  """A partial function call arguments."""

  json_path: Optional[str] = Field(
      default=None,
      description="""A JSON Path (RFC 9535) to the argument being streamed.
...
class FunctionCall(_common.BaseModel):
  ...
  partial_args: Optional[list[PartialArg]] = Field(
      default=None,
      description="""A list of partial function call arguments. If populated, the client
...
class FunctionCallDict(TypedDict, total=False):
  ...
  """A list of partial function call arguments. If populated, the client
      to execute the `function_call` and return the response with the matching
      `id`."""
...
class Schema(_common.BaseModel):
  ...
  one_of: Optional[list['Schema']] = Field(
      default=None,
      description="""Optional. The value should be validated against exactly one of the subschemas in the list.""",
  )
  ...
  all_of: Optional[list['Schema']] = Field(
      default=None,
      description="""Optional. The value should be validated against all of the subschemas in the list.""",
  )
  not_: Optional['Schema'] = Field(
      default=None,
      description="""Optional. The data value is not valid against the specified schema. In the below example pet_type value can be of any type except integer. type: object properties: pet_type: not: type: integer""",
  )
  additional_properties_schema: Optional['Schema'] = Field(
      default=None,
      description="""Optional. Field used internally (corresponding to `additional_properties`); must be a Schema.""",
  )
...
class SchemaDict(TypedDict, total=False):
  ...
  one_of: Optional[list['SchemaDict']]
  """Optional. The value should be validated against exactly one of the subschemas in the list."""

  any_of: Optional[list['SchemaDict']]
  """Optional. The value should be validated against any (one or more) of the subschemas in the list."""

  all_of: Optional[list['SchemaDict']]
  """Optional. The value should be validated against all of the subschemas in the list."""

  not_: Optional['SchemaDict']
  """Optional. The data value is not valid against the specified schema. In the below example pet_type value can be of any type except integer. type: object properties: pet_type: not: type: integer"""

  additional_properties_schema: Optional['SchemaDict']
  """Optional. Field used internally (corresponding to `additional_properties`); must be a Schema."""
...
class GoogleTypeInterval(_common.BaseModel):
...
class GoogleTypeIntervalDict(TypedDict, total=False):
  """Represents a time interval, encoded as a Timestamp start (inclusive) and a Timestamp end (exclusive).
...
GoogleTypeIntervalOrDict = Union[GoogleTypeInterval, GoogleTypeIntervalDict]
...
class GoogleSearchRetrieval(_common.BaseModel):
  ...
  time_range_filter: Optional[GoogleTypeInterval] = Field(
      default=None,
...
class GoogleSearchRetrievalDict(TypedDict, total=False):
  ...
  time_range_filter: Optional[GoogleTypeIntervalDict]
...
class AuthConfigClientCertificateConfig(_common.BaseModel):
  """Config for Client Certificate Authentication."""

  ssl_certificate: Optional[str] = Field(
      default=None,
      description="""Required. The name of the SecretManager secret version resource storing the ssl certificate encoded in PEM format. The seceret must include the begin header and end footer lines. Format: `projects/{project}/secrets/{secrete}/versions/{version}` - If specified, the `secretmanager.versions.access` permission should be granted to Vertex AI Extension Service Agent (https://cloud.google.com/vertex-ai/docs/general/access-control#service-agents) on the specified resource.""",
  )
  private_key: Optional[str] = Field(
      default=None,
      description="""Required. The name of the SecretManager secret version resource storing the private key encoded in PEM format. Format: `projects/{project}/secrets/{secrete}/versions/{version}` - If specified, the `secretmanager.versions.access` permission should be granted to Vertex AI Extension Service Agent (https://cloud.google.com/vertex-ai/docs/general/access-control#service-agents) on the specified resource.""",
  )
  passphrase: Optional[str] = Field(
      default=None,
      description="""Optional. The name of the SecretManager secret version resource storing the passphrase. 'passphrase' should be left unset if private key is not encrypted. Format: `projects/{project}/secrets/{secrete}/versions/{version}` - If specified, the `secretmanager.versions.access` permission should be granted to Vertex AI Extension Service Agent (https://cloud.google.com/vertex-ai/docs/general/access-control#service-agents) on the specified resource.""",
  )


class AuthConfigClientCertificateConfigDict(TypedDict, total=False):
  """Config for Client Certificate Authentication."""
  ...
  """Optional. The name of the SecretManager secret version resource storing the passphrase. 'passphrase' should be left unset if private key is not encrypted. Format: `projects/{project}/secrets/{secrete}/versions/{version}` - If specified, the `secretmanager.versions.access` permission should be granted to Vertex AI Extension Service Agent (https://cloud.google.com/vertex-ai/docs/general/access-control#service-agents) on the specified resource."""


AuthConfigClientCertificateConfigOrDict = Union[
    AuthConfigClientCertificateConfig, AuthConfigClientCertificateConfigDict
]
...
class AuthConfig(_common.BaseModel):
  ...
  client_certificate_config: Optional[AuthConfigClientCertificateConfig] = (
      Field(default=None, description="""Config for Client Certificate auth.""")
  )
...
class AuthConfigDict(TypedDict, total=False):
  ...
  client_certificate_config: Optional[AuthConfigClientCertificateConfigDict]
  """Config for Client Certificate auth."""
...
class FileSearchRetrievalResource(_common.BaseModel):
  """The file search resource to retrieve from."""

  rag_store_name: Optional[str] = Field(
      default=None,
      description="""The name of the rag store to retrieve from.
      Example: `ragStores/my-rag-store-123`""",
  )


class FileSearchRetrievalResourceDict(TypedDict, total=False):
  """The file search resource to retrieve from."""

  rag_store_name: Optional[str]
  """The name of the rag store to retrieve from.
      Example: `ragStores/my-rag-store-123`"""


FileSearchRetrievalResourceOrDict = Union[
    FileSearchRetrievalResource, FileSearchRetrievalResourceDict
]


class FileSearchRetrievalConfig(_common.BaseModel):
  """File search retrieval configuration."""
...
class FileSearchRetrievalConfigDict(TypedDict, total=False):
  """File search retrieval configuration."""
...
FileSearchRetrievalConfigOrDict = Union[
    FileSearchRetrievalConfig, FileSearchRetrievalConfigDict
]


class FileSearch(_common.BaseModel):
  """Tool to retrieve knowledge from the File Search rag stores."""

  retrieval_resources: Optional[list[FileSearchRetrievalResource]] = Field(
      default=None,
      description="""File search retrieval resources to retrieve from.""",
  )
  retrieval_config: Optional[FileSearchRetrievalConfig] = Field(
      default=None, description="""The configuration for the retrieval."""
  )
...
class FileSearchDict(TypedDict, total=False):
  ...
  retrieval_resources: Optional[list[FileSearchRetrievalResourceDict]]
  """File search retrieval resources to retrieve from."""

  retrieval_config: Optional[FileSearchRetrievalConfigDict]
  """The configuration for the retrieval."""
...
class VertexRagStoreRanking(_common.BaseModel):
...
class VertexRagStoreRankingDict(TypedDict, total=False):
  """Configurations for hybrid search results ranking."""
...
VertexRagStoreRankingOrDict = Union[
    VertexRagStoreRanking, VertexRagStoreRankingDict
]
...
class RagRetrievalConfigRanking(_common.BaseModel):
  ...
  rank_count: Optional[int] = Field(
      default=None,
      description="""Optional. The number of contexts to rank. If not set, fallback to `top_k`. Example: When `rank_count` = 20 and `top_k` = 10, 20 contexts will be retrieved and ranked and then top 10 contexts will be returned.""",
  )
...
class RagRetrievalConfigRankingDict(TypedDict, total=False):
  ...
  """Optional. The number of contexts to rank. If not set, fallback to `top_k`. Example: When `rank_count` = 20 and `top_k` = 10, 20 contexts will be retrieved and ranked and then top 10 contexts will be returned."""
...
class VertexRagStore(_common.BaseModel):
  ...
  ranking: Optional[VertexRagStoreRanking] = Field(
      default=None,
...
class VertexRagStoreDict(TypedDict, total=False):
  ...
  ranking: Optional[VertexRagStoreRankingDict]
...
class Retrieval(_common.BaseModel):
  ...
  name: Optional[str] = Field(
      default=None,
      description="""Optional. The name to identify the retrieval source. It can be empty or any string that contain only alphanumeric characters ([a-z0-9_]). It must be unique within the request.""",
  )
...
class RetrievalDict(TypedDict, total=False):
  ...
  name: Optional[str]
  """Optional. The name to identify the retrieval source. It can be empty or any string that contain only alphanumeric characters ([a-z0-9_]). It must be unique within the request."""
...
class Tool(_common.BaseModel):
  ...
  file_search: Optional[FileSearch] = Field(
      default=None,
      description="""Optional. Tool to retrieve knowledge from the File Search rag stores.""",
  )
...
class ToolDict(TypedDict, total=False):
  ...
  """Optional. Tool to retrieve knowledge from the File Search rag stores."""
...
class FunctionCallingConfigAllowedFunctionNames(_common.BaseModel):
  """Config for parallel function calls."""

  names: Optional[list[str]] = Field(
      default=None,
      description="""Optional. The names of the functions to call. Function names should match [FunctionDeclaration.name].""",
  )
  is_optional: Optional[bool] = Field(
      default=None,
      description="""Whether the function call is optional for this set of function names. If true, the model will predict at most one function call from this set of function names. If false, the model will predict exactly one function call from this set of function names.""",
  )


class FunctionCallingConfigAllowedFunctionNamesDict(TypedDict, total=False):
  """Config for parallel function calls."""

  names: Optional[list[str]]
  """Optional. The names of the functions to call. Function names should match [FunctionDeclaration.name]."""

  is_optional: Optional[bool]
  """Whether the function call is optional for this set of function names. If true, the model will predict at most one function call from this set of function names. If false, the model will predict exactly one function call from this set of function names."""


FunctionCallingConfigAllowedFunctionNamesOrDict = Union[
    FunctionCallingConfigAllowedFunctionNames,
    FunctionCallingConfigAllowedFunctionNamesDict,
]
...
class FunctionCallingConfig(_common.BaseModel):
  ...
  parallel_function_call_config: Optional[
      list[FunctionCallingConfigAllowedFunctionNames]
  ] = Field(
      default=None,
      description="""Optional. Sets of Function names to call. Only set when the Mode is ANY. With mode set to ANY, model will predict at most one function call from each set of function names provided. NOTE: parallel_function_call_config and allowed_function_names cannot both be set.""",
  )
...
class FunctionCallingConfigDict(TypedDict, total=False):
  ...
  parallel_function_call_config: Optional[
      list[FunctionCallingConfigAllowedFunctionNamesDict]
  ]
  """Optional. Sets of Function names to call. Only set when the Mode is ANY. With mode set to ANY, model will predict at most one function call from each set of function names provided. NOTE: parallel_function_call_config and allowed_function_names cannot both be set."""
...
class RetrievalConfig(_common.BaseModel):
  ...
  time_zone: Optional[str] = Field(
      default=None, description="""IANA time zone, e.g. Europe/Budapest."""
  )
...
class ToolConfig(_common.BaseModel):
  ...
  tool_turn_limit: Optional[int] = Field(
      default=None,
      description="""Optional. The ToolUseAgent maximum turn limit for the request.""",
  )
...
class ToolConfigDict(TypedDict, total=False):
  ...
  tool_turn_limit: Optional[int]
  """Optional. The ToolUseAgent maximum turn limit for the request."""
...
class CustomVoiceConfig(_common.BaseModel):
  """The configuration for the custom voice to use."""

  custom_voice_sample: Optional[bytes] = Field(
      default=None,
      description="""The sample of the custom voice, in pcm16 s16e format.""",
  )


class CustomVoiceConfigDict(TypedDict, total=False):
  """The configuration for the custom voice to use."""

  custom_voice_sample: Optional[bytes]
  """The sample of the custom voice, in pcm16 s16e format."""


CustomVoiceConfigOrDict = Union[CustomVoiceConfig, CustomVoiceConfigDict]
...
class VoiceConfig(_common.BaseModel):
  ...
  custom_voice_config: Optional[CustomVoiceConfig] = Field(
      default=None,
      description="""Optional. The configuration for the custom voice to use.""",
  )
...
class VoiceConfigDict(TypedDict, total=False):
  ...
  custom_voice_config: Optional[CustomVoiceConfigDict]
  """Optional. The configuration for the custom voice to use."""
...
class GroundingAttributionWeb(_common.BaseModel):
...
class GroundingAttributionWebDict(TypedDict, total=False):
  """Attribution from the web."""
...
GroundingAttributionWebOrDict = Union[
    GroundingAttributionWeb, GroundingAttributionWebDict
]


class GroundingAttributionRetrievedContext(_common.BaseModel):
...
class GroundingAttributionRetrievedContextDict(TypedDict, total=False):
  """Attribution from context retrieved by the retrieval tools."""
...
GroundingAttributionRetrievedContextOrDict = Union[
    GroundingAttributionRetrievedContext,
    GroundingAttributionRetrievedContextDict,
]
...
class GroundingAttribution(_common.BaseModel):
  """Grounding attribution. Deprecated. Use GroundingSupport instead."""

  web: Optional[GroundingAttributionWeb] = Field(
      default=None, description="""Optional. Attribution from the web."""
  )
  retrieved_context: Optional[GroundingAttributionRetrievedContext] = Field(
      default=None,
...
class GroundingAttributionDict(TypedDict, total=False):
  """Grounding attribution. Deprecated. Use GroundingSupport instead."""

  web: Optional[GroundingAttributionWebDict]
  """Optional. Attribution from the web."""

  retrieved_context: Optional[GroundingAttributionRetrievedContextDict]
...
GroundingAttributionOrDict = Union[
    GroundingAttribution, GroundingAttributionDict
]
...
class GroundingChunkRetrievedContext(_common.BaseModel):
  ...
  page_number: Optional[int] = Field(
      default=None,
      description="""Output only. Page number of the attribution, if applicable.""",
  )
...
class GroundingChunkRetrievedContextDict(TypedDict, total=False):
  ...
  """Output only. Page number of the attribution, if applicable."""
...
class GroundingChunk(_common.BaseModel):
  ...
  chunk_metadata: Optional[dict[str, str]] = Field(
      default=None,
      description="""More fine-grained metadata for the grounding chunk. Note: keep this field as internal, and should only be used for AgentSpace use cases.""",
  )
...
class GroundingChunkDict(TypedDict, total=False):
  ...
  """More fine-grained metadata for the grounding chunk. Note: keep this field as internal, and should only be used for AgentSpace use cases."""
...
class ImageContextSource(_common.BaseModel):
  """Metadata about the source of the image."""

  uri: Optional[str] = Field(
      default=None, description="""URI reference of the source chunk."""
  )
  title: Optional[str] = Field(
      default=None, description="""Title of the source chunk."""
  )


class ImageContextSourceDict(TypedDict, total=False):
  """Metadata about the source of the image."""

  uri: Optional[str]
  """URI reference of the source chunk."""

  title: Optional[str]
  """Title of the source chunk."""


ImageContextSourceOrDict = Union[ImageContextSource, ImageContextSourceDict]


class ImageContextImage(_common.BaseModel):
  """Metadata about the image."""

  uri: Optional[str] = Field(
      default=None, description="""URI reference of the image."""
  )
  width: Optional[int] = Field(
      default=None, description="""Width of the image."""
  )
  height: Optional[int] = Field(
      default=None, description="""Height of the image."""
  )


class ImageContextImageDict(TypedDict, total=False):
  """Metadata about the image."""

  uri: Optional[str]
  """URI reference of the image."""
...
ImageContextImageOrDict = Union[ImageContextImage, ImageContextImageDict]


class ImageContext(_common.BaseModel):
  """Image context."""

  source: Optional[ImageContextSource] = Field(
      default=None, description="""Metadata about the source of the image"""
  )
  image: Optional[ImageContextImage] = Field(
      default=None, description="""Metadata about the full size image."""
  )
  thumbnail: Optional[ImageContextImage] = Field(
      default=None, description="""Metadata about the thumbnail."""
  )


class ImageContextDict(TypedDict, total=False):
  """Image context."""

  source: Optional[ImageContextSourceDict]
  """Metadata about the source of the image"""

  image: Optional[ImageContextImageDict]
  """Metadata about the full size image."""

  thumbnail: Optional[ImageContextImageDict]
  """Metadata about the thumbnail."""


ImageContextOrDict = Union[ImageContext, ImageContextDict]


class Attachment(_common.BaseModel):
  """Attachments for the content."""

  image: Optional[ImageContext] = Field(
      default=None, description="""Data about the image."""
  )
...
class AttachmentDict(TypedDict, total=False):
  ...
  image: Optional[ImageContextDict]
...
class GroundingMetadata(_common.BaseModel):
  ...
  grounding_attributions: Optional[list[GroundingAttribution]] = Field(
      default=None, description="""Optional. List of grounding attributions."""
  )
  ...
  attachments: Optional[list[Attachment]] = Field(
      default=None,
      description="""Optional. Output only. Attachments for the content.""",
  )
...
class GroundingMetadataDict(TypedDict, total=False):
  ...
  grounding_attributions: Optional[list[GroundingAttributionDict]]
  ...
  """Optional. Output only. Attachments for the content."""
...
class Candidate(_common.BaseModel):
  ...
  score: Optional[float] = Field(
      default=None,
      description="""Output only. Sum of the per token logprobability score of the candidate.""",
  )
...
class CandidateDict(TypedDict, total=False):
  ...
  """Output only. Sum of the per token logprobability score of the candidate."""
...
class GenerateContentResponseUsageMetadataBillablleUsage(_common.BaseModel):
  """[Deprecated] Billable usage information."""

  text_count: Optional[int] = Field(
      default=None, description="""The number of text characters."""
  )
  image_count: Optional[int] = Field(
      default=None, description="""The number of images."""
  )
  video_duration_seconds: Optional[int] = Field(
      default=None, description="""The duration of video in seconds."""
  )
  audio_duration_seconds: Optional[int] = Field(
      default=None, description="""The duration of audio in seconds."""
  )


class GenerateContentResponseUsageMetadataBillablleUsageDict(
    TypedDict, total=False
):
  """[Deprecated] Billable usage information."""
...
GenerateContentResponseUsageMetadataBillablleUsageOrDict = Union[
    GenerateContentResponseUsageMetadataBillablleUsage,
    GenerateContentResponseUsageMetadataBillablleUsageDict,
]
...
class GenerateContentResponseUsageMetadata(_common.BaseModel):
  ...
  billable_prompt_usage: Optional[
      GenerateContentResponseUsageMetadataBillablleUsage
  ] = Field(
      default=None,
      description="""Output only. [Deprecated] Billable usage for the prompt.""",
  )
  billable_cached_content_usage: Optional[
      GenerateContentResponseUsageMetadataBillablleUsage
  ] = Field(
      default=None,
      description="""Output only. [Deprecated] Billable usage for the cached content.""",
  )
...
class GenerateContentResponseInternalDebugInfo(_common.BaseModel):
  """Model optimizer debug info."""

  routed_model_name: Optional[str] = Field(
      default=None, description="""The name of the routed model."""
  )


class GenerateContentResponseInternalDebugInfoDict(TypedDict, total=False):
  """Model optimizer debug info."""

  routed_model_name: Optional[str]
  """The name of the routed model."""


GenerateContentResponseInternalDebugInfoOrDict = Union[
    GenerateContentResponseInternalDebugInfo,
    GenerateContentResponseInternalDebugInfoDict,
]
...
class GenerateContentResponse(_common.BaseModel):
  ...
  send_time: Optional[datetime.datetime] = Field(
      default=None,
      description="""Output only. Time at which the message was sent.""",
  )
  internal_debug_info: Optional[GenerateContentResponseInternalDebugInfo] = (
      Field(
          default=None,
          description="""Output only. Model optimizer debug info for internal users.""",
      )
  )
...
class GenerateContentResponseDict(TypedDict, total=False):
  ...
  send_time: Optional[datetime.datetime]
  """Output only. Time at which the message was sent."""

  internal_debug_info: Optional[GenerateContentResponseInternalDebugInfoDict]
  """Output only. Model optimizer debug info for internal users."""
...
class GenerationConfigContextConfig(_common.BaseModel):
  """Config related to the context window for the generation request.

  This may include configurations such as: * Whether we apply sliding window to
  the historical context, and what is the expected window size. * Whether we
  include video frames all the time, or only when there is a sentence detected
  from the audio.
  """

  video_frame_only_during_audio_activity: Optional[bool] = Field(
      default=None,
      description="""If true, video frames will only be included when there is a sentence detected from the audio.""",
  )


class GenerationConfigContextConfigDict(TypedDict, total=False):
  """Config related to the context window for the generation request.
  ...
  video_frame_only_during_audio_activity: Optional[bool]
  """If true, video frames will only be included when there is a sentence detected from the audio."""


GenerationConfigContextConfigOrDict = Union[
    GenerationConfigContextConfig, GenerationConfigContextConfigDict
]
...
class GenerationConfig(_common.BaseModel):
  ...
  logit_bias: Optional[dict[str, float]] = Field(
      default=None, description="""Optional. Logit bias."""
  )
  echo: Optional[bool] = Field(default=None, description="""Optional. Echo.""")
  ...
  min_p: Optional[float] = Field(
      default=None,
      description="""Optional. If specified, min-p sampling will be used.""",
  )
  audio_timestamp: Optional[bool] = Field(
      default=None,
  ...
  context_config: Optional[GenerationConfigContextConfig] = Field(
      default=None,
      description="""Optional. The context config for the generation request.""",
  )
...
class GenerationConfigDict(TypedDict, total=False):
  ...
  """Optional. If specified, min-p sampling will be used."""
  ...
  context_config: Optional[GenerationConfigContextConfigDict]
  """Optional. The context config for the generation request."""
...
class ReinforcementTuningUserDatasetExamples(_common.BaseModel):
  ...
  The contents are truncated for better UI showing.
  """
...
class ReinforcementLearningDataStats(_common.BaseModel):
  """Statistics computed for datasets used for reinforcement learning."""

  prompt_dataset_stats: Optional[DatasetStats] = Field(
      default=None,
      description="""Output only. Statistics computed for the prompt dataset used during reinforcement learning.""",
  )
  preference_dataset_stats: Optional[DatasetStats] = Field(
      default=None,
      description="""Output only. Statistics computed for the preference dataset. This can be either a human preference dataset or a preference dataset generated from AI feedback.""",
  )


class ReinforcementLearningDataStatsDict(TypedDict, total=False):
  """Statistics computed for datasets used for reinforcement learning."""

  prompt_dataset_stats: Optional[DatasetStatsDict]
  """Output only. Statistics computed for the prompt dataset used during reinforcement learning."""

  preference_dataset_stats: Optional[DatasetStatsDict]
...
ReinforcementLearningDataStatsOrDict = Union[
    ReinforcementLearningDataStats, ReinforcementLearningDataStatsDict
]


class DistillationPromptLabelResultStatsFilteredReasonDistribution(
    _common.BaseModel
):
  """Distribution of filtered out examples in the batch prediction results."""

  count: Optional[int] = Field(
      default=None,
      description="""Output only. Number of examples be filtered out for this reason.""",
  )
  reason: Optional[FilteredReason] = Field(
      default=None,
      description="""Output only. The reason for filtering out the example.""",
  )


class DistillationPromptLabelResultStatsFilteredReasonDistributionDict(
    TypedDict, total=False
):
  """Distribution of filtered out examples in the batch prediction results."""

  count: Optional[int]
  """Output only. Number of examples be filtered out for this reason."""

  reason: Optional[FilteredReason]
  """Output only. The reason for filtering out the example."""


DistillationPromptLabelResultStatsFilteredReasonDistributionOrDict = Union[
    DistillationPromptLabelResultStatsFilteredReasonDistribution,
    DistillationPromptLabelResultStatsFilteredReasonDistributionDict,
]


class DistillationPromptLabelResultStats(_common.BaseModel):
  """Statistics computed for prompt label result from batch prediction response."""

  total_prompt_label_result_count: Optional[int] = Field(
      default=None,
      description="""Output only. Number of examples in the batch prediction results for prompt label.""",
  )
  prompt_label_result_has_label_count: Optional[int] = Field(
      default=None,
      description="""Output only. Number of examples with label in the batch prediction results.""",
  )
  prompt_label_result_no_label_count: Optional[int] = Field(
      default=None,
      description="""Output only. Number of examples without label in the batch prediction results.""",
  )
  prompt_label_result_filtered_out_count: Optional[int] = Field(
      default=None,
      description="""Output only. Number of examples be filtered out in the batch prediction results.""",
  )
  prompt_label_result_filtered_reason_distribution: Optional[
      list[DistillationPromptLabelResultStatsFilteredReasonDistribution]
  ] = Field(
      default=None,
      description="""Output only. Filtered out examples reason in the batch prediction results.""",
  )


class DistillationPromptLabelResultStatsDict(TypedDict, total=False):
  """Statistics computed for prompt label result from batch prediction response."""

  total_prompt_label_result_count: Optional[int]
  """Output only. Number of examples in the batch prediction results for prompt label."""

  prompt_label_result_has_label_count: Optional[int]
  """Output only. Number of examples with label in the batch prediction results."""

  prompt_label_result_no_label_count: Optional[int]
  """Output only. Number of examples without label in the batch prediction results."""

  prompt_label_result_filtered_out_count: Optional[int]
  """Output only. Number of examples be filtered out in the batch prediction results."""

  prompt_label_result_filtered_reason_distribution: Optional[
      list[DistillationPromptLabelResultStatsFilteredReasonDistributionDict]
  ]
  """Output only. Filtered out examples reason in the batch prediction results."""


DistillationPromptLabelResultStatsOrDict = Union[
    DistillationPromptLabelResultStats, DistillationPromptLabelResultStatsDict
]
...
class DistillationDataStats(_common.BaseModel):
  ...
  prompt_label_stats: Optional[DistillationPromptLabelResultStats] = Field(
      default=None,
      description="""Output only. Statistics computed for the prompt label from batch prediction response.""",
  )
...
class DistillationDataStatsDict(TypedDict, total=False):
  ...
  prompt_label_stats: Optional[DistillationPromptLabelResultStatsDict]
  """Output only. Statistics computed for the prompt label from batch prediction response."""
...
class TuningDataStats(_common.BaseModel):
  ...
  reinforcement_learning_data_stats: Optional[
      ReinforcementLearningDataStats
  ] = Field(
      default=None,
      description="""Output only. Statistics for reinforcement learning.""",
  )
...
class TuningDataStatsDict(TypedDict, total=False):
  ...
  reinforcement_learning_data_stats: Optional[
      ReinforcementLearningDataStatsDict
  ]
  """Output only. Statistics for reinforcement learning."""
...
class DistillationHyperParameters(_common.BaseModel):
  ...
  sft_loss_weight_multiplier: Optional[float] = Field(
      default=None,
      description="""Optional. Multiplier for adjusting the weight of the Sequence-Level KD (can be viewed as supervised training on teacher-generated output) in the distillation. When set to 0, the loss is reduced to on-policy distillation.""",
  )
...
class DistillationHyperParametersDict(TypedDict, total=False):
  ...
  """Optional. Multiplier for adjusting the weight of the Sequence-Level KD (can be viewed as supervised training on teacher-generated output) in the distillation. When set to 0, the loss is reduced to on-policy distillation."""
...
class DistillationSpec(_common.BaseModel):
  ...
  labeled_dataset_uri: Optional[str] = Field(
      default=None,
      description="""Optional. Cloud Storage path to file containing labeled dataset for distillation. The dataset must be formatted as a JSONL file.""",
  )
  validation_dataset_uri: Optional[str] = Field(
      default=None,
      description="""Optional. Cloud Storage path to file containing validation dataset for tuning. The dataset must be formatted as a JSONL file.""",
...
class DistillationSpecDict(TypedDict, total=False):
  ...
  labeled_dataset_uri: Optional[str]
  """Optional. Cloud Storage path to file containing labeled dataset for distillation. The dataset must be formatted as a JSONL file."""
...
class HumanFeedbackConfig(_common.BaseModel):
  """Configures Reinforcement Learning to use human feedback during tuning."""

  preference_dataset_uri: Optional[str] = Field(
      default=None,
      description="""Required. Cloud Storage path to human preference data.""",
  )


class HumanFeedbackConfigDict(TypedDict, total=False):
  """Configures Reinforcement Learning to use human feedback during tuning."""

  preference_dataset_uri: Optional[str]
  """Required. Cloud Storage path to human preference data."""


HumanFeedbackConfigOrDict = Union[HumanFeedbackConfig, HumanFeedbackConfigDict]


class AiFeedbackConfig(_common.BaseModel):
  """Configures Reinforcement Learning to generate AI feedback during tuning."""

  prompt_dataset_uri: Optional[str] = Field(
      default=None,
      description="""Required. Cloud Storage path to prompt dataset used to generate AI preference data.""",
  )
  task_type: Optional[AiFeedbackTaskType] = Field(
      default=None,
      description="""Required. Type of task used to judge AI feedback. The best AI-generated candidate is chosen based on overall metrics for computed for a given task. Full criteria are described in the TaskType.""",
  )


class AiFeedbackConfigDict(TypedDict, total=False):
  """Configures Reinforcement Learning to generate AI feedback during tuning."""

  prompt_dataset_uri: Optional[str]
  """Required. Cloud Storage path to prompt dataset used to generate AI preference data."""

  task_type: Optional[AiFeedbackTaskType]
  """Required. Type of task used to judge AI feedback. The best AI-generated candidate is chosen based on overall metrics for computed for a given task. Full criteria are described in the TaskType."""


AiFeedbackConfigOrDict = Union[AiFeedbackConfig, AiFeedbackConfigDict]


class RewardModelTrainingConfig(_common.BaseModel):
  """Configures Reinforcement Learning to learn preference by training a reward model."""

  epoch_count: Optional[int] = Field(
      default=None,
      description="""Optional. Number of training epoches for the reward model training job.""",
  )
  learning_rate_multiplier: Optional[float] = Field(
      default=None,
      description="""Optional. Learning rate multiplier for reward model training.""",
  )


class RewardModelTrainingConfigDict(TypedDict, total=False):
  """Configures Reinforcement Learning to learn preference by training a reward model."""
...
RewardModelTrainingConfigOrDict = Union[
    RewardModelTrainingConfig, RewardModelTrainingConfigDict
]


class ReinforcementLearningHyperParameters(_common.BaseModel):
  """Hyperparameters for Reinforcement Learning."""

  human_feedback_config: Optional[HumanFeedbackConfig] = Field(
      default=None,
      description="""Configures Reinforcement Learning to use human feedback for preference data during tuning.""",
  )
  ai_feedback_config: Optional[AiFeedbackConfig] = Field(
      default=None,
      description="""Configures Reinforcement Learning to use AI feedback to generate preference data.""",
  )
  reward_model_training_config: Optional[RewardModelTrainingConfig] = Field(
      default=None,
      description="""Configures Reinforcement Learning to train a reward model to learn preference.""",
  )
  epoch_count: Optional[int] = Field(
      default=None,
  ...
  learning_rate_multiplier: Optional[float] = Field(
      default=None,
      description="""Optional. Learning rate multiplier for Reinforcement Learning.""",
  )
  kl_coefficient: Optional[float] = Field(
      default=None,
      description="""Optional. KL divergence coefficient for Reinforcement Learning.""",
  )


class ReinforcementLearningHyperParametersDict(TypedDict, total=False):
  """Hyperparameters for Reinforcement Learning."""

  human_feedback_config: Optional[HumanFeedbackConfigDict]
  """Configures Reinforcement Learning to use human feedback for preference data during tuning."""

  ai_feedback_config: Optional[AiFeedbackConfigDict]
  """Configures Reinforcement Learning to use AI feedback to generate preference data."""

  reward_model_training_config: Optional[RewardModelTrainingConfigDict]
  ...
  kl_coefficient: Optional[float]
  """Optional. KL divergence coefficient for Reinforcement Learning."""


ReinforcementLearningHyperParametersOrDict = Union[
    ReinforcementLearningHyperParameters,
    ReinforcementLearningHyperParametersDict,
]


class ReinforcementLearningSpec(_common.BaseModel):
  """Tuning Spec for Reinforcement Learning."""

  prompt_dataset_uri: Optional[str] = Field(
      default=None,
      description="""Required. Cloud Storage path to the prompt dataset to use during Reinforcement Learning.""",
  )
  validation_dataset_uri: Optional[str] = Field(
      default=None,
      description="""Optional. Cloud Storage path to the validation dataset to use during Reinforcement Learning.""",
  )
  hyper_parameters: Optional[ReinforcementLearningHyperParameters] = Field(
      default=None,
      description="""Optional. Additional hyper-parameters to use during tuning.""",
  )


class ReinforcementLearningSpecDict(TypedDict, total=False):
  """Tuning Spec for Reinforcement Learning."""
  ...
  hyper_parameters: Optional[ReinforcementLearningHyperParametersDict]
  """Optional. Additional hyper-parameters to use during tuning."""


ReinforcementLearningSpecOrDict = Union[
    ReinforcementLearningSpec, ReinforcementLearningSpecDict
]
...
class PreferenceOptimizationHyperParameters(_common.BaseModel):
  ...
  alpha: Optional[float] = Field(
      default=None,
      description="""Optional. Weight for preferred examples, 1 - alpha is the weight for non-preferred examples.""",
  )
...
class PreferenceOptimizationHyperParametersDict(TypedDict, total=False):
  ...
  alpha: Optional[float]
  """Optional. Weight for preferred examples, 1 - alpha is the weight for non-preferred examples."""
...
class ReinforcementTuningCodeExecutionRewardScorer(_common.BaseModel):
  """Expects the user to implement the following function: ``` def evaluate(prompt:str, response:str, references: Dict[str,str], system_instruction:str) -> float: ``` where the first returned argument is reward.
  ...
  python_code_snippet: Optional[str] = Field(
      default=None,
      description="""Example python code snippet which assigns reward of 1 to answer matching user provided reference answer in per prompt references map. ``` def evaluate(prompt:list[contents], response:str, references: Mapping[str,str], system_instruction:str) -> float: if response == references["reference_key"]: return 1.0 return -1.0 ``` Important: reward output by the function is clipped to be within [-1, 1]. I.e., reward = max(min(reward, 1), -1)""",
  )
...
class ReinforcementTuningCodeExecutionRewardScorerDict(TypedDict, total=False):
  ...
  """Example python code snippet which assigns reward of 1 to answer matching user provided reference answer in per prompt references map. ``` def evaluate(prompt:list[contents], response:str, references: Mapping[str,str], system_instruction:str) -> float: if response == references["reference_key"]: return 1.0 return -1.0 ``` Important: reward output by the function is clipped to be within [-1, 1]. I.e., reward = max(min(reward, 1), -1)"""
...
class ReinforcementTuningStringMatchRewardScorerStringMatchExpression(
  ...
  Evaluator returns `true` if `MatchOperation(target, expression)` evaluates to
...
class ReinforcementTuningStringMatchRewardScorerJsonMatchExpression(
  ...
  """JsonRewardScorer supports converting parsed responses to JSON format, find the first-level key in json matching key name, then perform `StringMatchRewardScorer` operation on the value of matching key name."""
...
class ReinforcementTuningStringMatchRewardScorer(_common.BaseModel):
  ...
  For example, for math problems, we can use string match scorer to check if the
  correct exact answer is generated.
  """
  ...
  correct_answer_reward: Optional[float] = Field(
      default=None,
      description="""Correct answer rewawrd is returned if evaluator evaluates to `true`. All correct answers get the same reward.""",
  )
...
class ReinforcementTuningStringMatchRewardScorerDict(TypedDict, total=False):
  ...
  """Correct answer rewawrd is returned if evaluator evaluates to `true`. All correct answers get the same reward."""
...
class ReinforcementTuningAutoraterScorerParsedResponseConversionScorer(
  ...
  Important: reward is clipped to be within [-1, 1]. I.e., reward =
  max(min(reward, 1), -1)
  """
...
class ReinforcementTuningAutoraterScorerExactMatchScorerDict(
  ...
  expression: Optional[str]
  """The string expression to match against. Supports substitution in the format of {{references.reference}} before matching. No regex support. For example, customer can define: ``` correct_answer_reward: 1.0 wrong_answer_reward: -1.0 expression: {{references.reference}} ``` And during evaluation for each parsed autorater response, the prompt references will be substituted into the expression here. If references.reference has a value of "3", the expression becomes "3" and we will give 1.0 reward if parsed response string equals "3". and -1.0 reward if parsed response string does not equal "3"."""
...
class ReinforcementTuningParseResponseConfig(_common.BaseModel):
  ...
  to problem A, finally output answer in ` ` block." And the sample response
  might look like: "`blahblah`". Here, user can define the following parse
  config: ``` parse_type: REGEX_EXTRACT regex_extract_expression: ".*(.*?)" ```
  And we would have returned "blahblah" to reward scoring function.
  """
...
class ReinforcementTuningAutoraterScorer(_common.BaseModel):
  ...
  For example, for math problems, we can use classification based autorater to
  calculate the reward based on the autorater parsed response against reference
  answer.
  """
  ...
  parsed_response_conversion_scorer: Optional[
      ...
      default=None,
      description="""Scores autorater responses by directly converting parsed autorater response to float reward. Important: reward is clipped to be within [-1, 1]. I.e., reward = max(min(reward, 1), -1)""",
  )
  ...
  autorater_config: Optional[AutoraterConfig] = Field(
      default=None,
      description="""Autorater config for classification based autorater""",
  )
  autorater_response_parse_config: Optional[
      ReinforcementTuningParseResponseConfig
  ] = Field(
      default=None,
      description="""Parses autorater returned response. For example, if autorater response has reward stored in `2.0` block, defining parsing response config using regex "`.*(.*?)`" will give you "2.0".""",
  )
...
class ReinforcementTuningAutoraterScorerDict(TypedDict, total=False):
  ...
  """Scores autorater responses by directly converting parsed autorater response to float reward. Important: reward is clipped to be within [-1, 1]. I.e., reward = max(min(reward, 1), -1)"""
  ...
  """Parses autorater returned response. For example, if autorater response has reward stored in `2.0` block, defining parsing response config using regex "`.*(.*?)`" will give you "2.0"."""
...
class ReinforcementTuningCloudRunRewardScorer(_common.BaseModel):
  """The Cloud Run service should implement the following HTTP API: ``` HTTP Method: POST HTTP Request Body: { "example": ReinforcementTuningExample, "response": Content, } ``` where `example` is a ReinforcementTuningExample in ProtoJSON format and `response` is a Content in ProtoJSON format.

  ``` HTTP Response Body: { "reward": float } Example HTTP Request Body: {
  "example": { "contents": [ { "role": "user", "parts": [ { "text": "What is the
  capital of France?" } ] } ], "references": { "answer": "Paris", } },
  "response": { "parts": [ { "text": "London" } ] } } Example HTTP Response
  ...
  cloud_run_uri: Optional[str] = Field(
      default=None,
      description="""URI of the Cloud Run service that will be used to compute the reward. The Vertex AI Secure Fine Tuning Service Agent (`service-@gcp-sa-vertex-tune.iam.gserviceaccount.com`) must be granted the permission (e.g. by granting `roles/run.invoker` in IAM) to invoke the Cloud Run service.""",
  )
...
class ReinforcementTuningCloudRunRewardScorerDict(TypedDict, total=False):
  ...
  """URI of the Cloud Run service that will be used to compute the reward. The Vertex AI Secure Fine Tuning Service Agent (`service-@gcp-sa-vertex-tune.iam.gserviceaccount.com`) must be granted the permission (e.g. by granting `roles/run.invoker` in IAM) to invoke the Cloud Run service."""
...
class SingleReinforcementTuningRewardConfig(_common.BaseModel):
  ...
  Each reward calculation/evaluation consists of two stages: stage 1: parse the
  part of information important from sample response via regex extract or simply
  take the sample response unmodified. stage 2: Call specific reward scorer to
  compute the reward and also output whether the sample answer is correct. While
  wrong answer and correct answer should get assigned different rewards, correct
  answers could also get assigned different rewards.
  """
  ...
  string_match_reward_scorer: Optional[
      ...
      default=None,
      description="""ReinforcementTuningClassificationBasedAutorater is used to score parsed responses for simple string matching use cases against reference answer without writing python code.""",
  )
  autorater_scorer: Optional[ReinforcementTuningAutoraterScorer] = Field(
      default=None,
      description="""ReinforcementTuningScoreBasedAutorater is used to score parsed responses based on score computed by autorater.""",
  )
  ...
  parse_response_config: Optional[ReinforcementTuningParseResponseConfig] = (
      Field(
          default=None,
          description="""Defines how to parse sample response. For example, we might want to extract only text between tags and we want text between the last ` ` tags in case there are multiple such tags. We could define a regex of "`.*(.*?)`" to parse the sample response.""",
      )
  )
...
class SingleReinforcementTuningRewardConfigDict(TypedDict, total=False):
  ...
  """ReinforcementTuningClassificationBasedAutorater is used to score parsed responses for simple string matching use cases against reference answer without writing python code."""

  autorater_scorer: Optional[ReinforcementTuningAutoraterScorerDict]
  """ReinforcementTuningScoreBasedAutorater is used to score parsed responses based on score computed by autorater."""
  ...
  """Defines how to parse sample response. For example, we might want to extract only text between tags and we want text between the last ` ` tags in case there are multiple such tags. We could define a regex of "`.*(.*?)`" to parse the sample response."""
...
class CompositeReinforcementTuningRewardConfigWeightedRewardConfig(
  ...
  """Reward function configuration with a weight.

  The weight is used to combine the reward with other rewards. The weight can be
  overridden at the data source level with dataset specific weights, which is a
  map from reward_name to reward_weight.
  """
...
class ReinforcementTuningHyperParameters(_common.BaseModel):
  ...
  thinking_budget: Optional[int] = Field(
      default=None,
      description="""Optional. The thinking budget for the tuning job to optimize for. -1 means dynamic thinking 0 means no thinking >0 means thinking budget in tokens""",
  )
  batch_size: Optional[int] = Field(
      default=None,
...
class ReinforcementTuningHyperParametersDict(TypedDict, total=False):
  ...
  thinking_budget: Optional[int]
  """Optional. The thinking budget for the tuning job to optimize for. -1 means dynamic thinking 0 means no thinking >0 means thinking budget in tokens"""
...
class ReinforcementTuningSpec(_common.BaseModel):
  ...
  composited_reward_config: Optional[
      CompositeReinforcementTuningRewardConfig
  ] = Field(
      default=None,
...
class EvaluationDataset(_common.BaseModel):
  ...
  is_unlimited_size: Optional[bool] = Field(
...
class EvaluationDatasetDict(TypedDict, total=False):
  ...
  is_unlimited_size: Optional[bool]
  """Optional. """
...
class EvaluateDatasetMetadata(_common.BaseModel):
  """Metadata for EvaluationService.EvaluateDataset."""

  successful_instances_count: Optional[int] = Field(
      default=None,
      description="""Number of instances that EvaluateDataset generated all metrics successfully.""",
  )
  partial_successful_instances_count: Optional[int] = Field(
      default=None,
      description="""Number of instances that EvaluateDataset generated partial metrics successfully.""",
  )
  failed_instances_count: Optional[int] = Field(
      default=None,
      description="""Number of instances that EvaluateDataset failed to generate all metrics.""",
  )


class EvaluateDatasetMetadataDict(TypedDict, total=False):
  """Metadata for EvaluationService.EvaluateDataset."""

  successful_instances_count: Optional[int]
  """Number of instances that EvaluateDataset generated all metrics successfully."""

  partial_successful_instances_count: Optional[int]
  """Number of instances that EvaluateDataset generated partial metrics successfully."""

  failed_instances_count: Optional[int]
  """Number of instances that EvaluateDataset failed to generate all metrics."""


EvaluateDatasetMetadataOrDict = Union[
    EvaluateDatasetMetadata, EvaluateDatasetMetadataDict
]
...
class EvaluateDatasetResponse(_common.BaseModel):
  ...
  metadata: Optional[EvaluateDatasetMetadata] = Field(
      default=None,
      description="""Metadata for EvaluationService.EvaluateDataset.""",
  )
...
class EvaluateDatasetResponseDict(TypedDict, total=False):
  ...
  metadata: Optional[EvaluateDatasetMetadataDict]
  """Metadata for EvaluationService.EvaluateDataset."""
...
class TuningJob(_common.BaseModel):
  ...
  reinforcement_learning_spec: Optional[ReinforcementLearningSpec] = Field(
      default=None, description="""Tuning Spec for Reinforcement Learning."""
  )
...
class TuningJobDict(TypedDict, total=False):
  ...
  reinforcement_learning_spec: Optional[ReinforcementLearningSpecDict]
  """Tuning Spec for Reinforcement Learning."""
...
class CustomMetadata(_common.BaseModel):
  ...
  key: Optional[str] = Field(
      default=None, description="""The key of the metadata to store."""
  )
  string_value: Optional[str] = Field(
      default=None, description="""The string value of the metadata to store."""
  )
  string_list_value: Optional[list[str]] = Field(
      default=None,
...
class Document(_common.BaseModel):
  ...
  custom_metadata: Optional[list[CustomMetadata]] = Field(
      default=None,
      description="""User provided custom metadata stored as key-value pairs.""",
  )
  update_time: Optional[datetime.datetime] = Field(
      default=None,
...
class _ListDocumentsParameters(_common.BaseModel):
  ...
  parent: Optional[str] = Field(
      default=None,
      description="""The resource name of the RagStore. Example: `ragStores/rag-store-foo`""",
  )
...
class _ListDocumentsParametersDict(TypedDict, total=False):
  ...
  parent: Optional[str]
  """The resource name of the RagStore. Example: `ragStores/rag-store-foo`"""
...
class ListDocumentsResponse(_common.BaseModel):
  ...
  next_page_token: Optional[str] = Field(default=None, description="""""")
  documents: Optional[list[Document]] = Field(default=None, description="""""")
...
class CreateRagStoreConfig(_common.BaseModel):
  """Optional parameters for creating a RagStore."""
  ...
  display_name: Optional[str] = Field(
      default=None,
      description="""The human-readable display name for the RagStore.
      """,
  )


class CreateRagStoreConfigDict(TypedDict, total=False):
  """Optional parameters for creating a RagStore."""
  ...
  display_name: Optional[str]
  """The human-readable display name for the RagStore.
      """


CreateRagStoreConfigOrDict = Union[
    CreateRagStoreConfig, CreateRagStoreConfigDict
]


class _CreateRagStoreParameters(_common.BaseModel):
  """Config for rag_stores.create parameters."""

  config: Optional[CreateRagStoreConfig] = Field(
      default=None,
      description="""Optional parameters for creating a RagStore.
      """,
  )


class _CreateRagStoreParametersDict(TypedDict, total=False):
  """Config for rag_stores.create parameters."""

  config: Optional[CreateRagStoreConfigDict]
  """Optional parameters for creating a RagStore.
      """


_CreateRagStoreParametersOrDict = Union[
    _CreateRagStoreParameters, _CreateRagStoreParametersDict
]


class RagStore(_common.BaseModel):
  """A collection of Documents."""

  name: Optional[str] = Field(
      default=None,
      description="""The resource name of the RagStore. Example: `ragStores/my-rag-store-123`""",
  )
  display_name: Optional[str] = Field(
      default=None,
      description="""The human-readable display name for the RagStore.""",
  ...
  create_time: Optional[datetime.datetime] = Field(
      default=None,
      description="""The Timestamp of when the RagStore was created.""",
  )
  update_time: Optional[datetime.datetime] = Field(
      default=None,
      description="""The Timestamp of when the RagStore was last updated.""",
  )


class RagStoreDict(TypedDict, total=False):
  """A collection of Documents."""

  name: Optional[str]
  """The resource name of the RagStore. Example: `ragStores/my-rag-store-123`"""

  display_name: Optional[str]
  """The human-readable display name for the RagStore."""

  create_time: Optional[datetime.datetime]
  """The Timestamp of when the RagStore was created."""

  update_time: Optional[datetime.datetime]
  """The Timestamp of when the RagStore was last updated."""


RagStoreOrDict = Union[RagStore, RagStoreDict]


class GetRagStoreConfig(_common.BaseModel):
  """Optional parameters for getting a RagStore."""
...
class GetRagStoreConfigDict(TypedDict, total=False):
  """Optional parameters for getting a RagStore."""
...
GetRagStoreConfigOrDict = Union[GetRagStoreConfig, GetRagStoreConfigDict]


class _GetRagStoreParameters(_common.BaseModel):
  """Config for rag_stores.get parameters."""

  name: Optional[str] = Field(
      default=None,
      description="""The resource name of the RagStore. Example: `ragStores/my-rag-store-123`""",
  )
  config: Optional[GetRagStoreConfig] = Field(
      default=None, description="""Optional parameters for the request."""
  )


class _GetRagStoreParametersDict(TypedDict, total=False):
  """Config for rag_stores.get parameters."""

  name: Optional[str]
  """The resource name of the RagStore. Example: `ragStores/my-rag-store-123`"""

  config: Optional[GetRagStoreConfigDict]
  """Optional parameters for the request."""


_GetRagStoreParametersOrDict = Union[
    _GetRagStoreParameters, _GetRagStoreParametersDict
]


class DeleteRagStoreConfig(_common.BaseModel):
  """Optional parameters for deleting a RagStore."""
  ...
  force: Optional[bool] = Field(
      default=None,
      description="""If set to true, any Documents and objects related to this RagStore will also be deleted.
      If false (the default), a FAILED_PRECONDITION error will be returned if
      the RagStore contains any Documents.
      """,
  )


class DeleteRagStoreConfigDict(TypedDict, total=False):
  """Optional parameters for deleting a RagStore."""
  ...
  force: Optional[bool]
  """If set to true, any Documents and objects related to this RagStore will also be deleted.
      If false (the default), a FAILED_PRECONDITION error will be returned if
      the RagStore contains any Documents.
      """


DeleteRagStoreConfigOrDict = Union[
    DeleteRagStoreConfig, DeleteRagStoreConfigDict
]


class _DeleteRagStoreParameters(_common.BaseModel):
  """Config for rag_stores.delete parameters."""

  name: Optional[str] = Field(
      default=None,
      description="""The resource name of the RagStore. Example: `ragStores/my-rag-store-123`""",
  )
  config: Optional[DeleteRagStoreConfig] = Field(
      default=None, description="""Optional parameters for the request."""
  )


class _DeleteRagStoreParametersDict(TypedDict, total=False):
  """Config for rag_stores.delete parameters."""

  name: Optional[str]
  """The resource name of the RagStore. Example: `ragStores/my-rag-store-123`"""

  config: Optional[DeleteRagStoreConfigDict]
  """Optional parameters for the request."""


_DeleteRagStoreParametersOrDict = Union[
    _DeleteRagStoreParameters, _DeleteRagStoreParametersDict
]


class ListRagStoresConfig(_common.BaseModel):
  """Optional parameters for listing RagStores."""
...
class ListRagStoresConfigDict(TypedDict, total=False):
  """Optional parameters for listing RagStores."""
...
ListRagStoresConfigOrDict = Union[ListRagStoresConfig, ListRagStoresConfigDict]


class _ListRagStoresParameters(_common.BaseModel):
  """Config for rag_stores.list parameters."""

  config: Optional[ListRagStoresConfig] = Field(
      default=None, description="""Optional parameters for the list request."""
  )


class _ListRagStoresParametersDict(TypedDict, total=False):
  """Config for rag_stores.list parameters."""

  config: Optional[ListRagStoresConfigDict]
  """Optional parameters for the list request."""


_ListRagStoresParametersOrDict = Union[
    _ListRagStoresParameters, _ListRagStoresParametersDict
]


class ListRagStoresResponse(_common.BaseModel):
  """Config for rag_stores.list return value."""

  sdk_http_response: Optional[HttpResponse] = Field(
      default=None, description="""Used to retain the full HTTP response."""
  )
  next_page_token: Optional[str] = Field(default=None, description="""""")
  rag_stores: Optional[list[RagStore]] = Field(
      default=None, description="""The returned rag stores."""
  )


class ListRagStoresResponseDict(TypedDict, total=False):
  """Config for rag_stores.list return value."""
  ...
  rag_stores: Optional[list[RagStoreDict]]
  """The returned rag stores."""


ListRagStoresResponseOrDict = Union[
    ListRagStoresResponse, ListRagStoresResponseDict
]
...
class UploadToRagStoreConfig(_common.BaseModel):
  """Optional parameters for uploading a file to a RagStore."""
...
class UploadToRagStoreConfigDict(TypedDict, total=False):
  """Optional parameters for uploading a file to a RagStore."""
...
UploadToRagStoreConfigOrDict = Union[
    UploadToRagStoreConfig, UploadToRagStoreConfigDict
]


class _UploadToRagStoreParameters(_common.BaseModel):
  """Generates the parameters for the private _upload_to_rag_store method."""

  rag_store_name: Optional[str] = Field(
      default=None,
      description="""The resource name of the RagStore. Example: `ragStores/my-rag-store-123`""",
  )
  config: Optional[UploadToRagStoreConfig] = Field(
      default=None,
      description="""Used to override the default configuration.""",
  )


class _UploadToRagStoreParametersDict(TypedDict, total=False):
  """Generates the parameters for the private _upload_to_rag_store method."""

  rag_store_name: Optional[str]
  """The resource name of the RagStore. Example: `ragStores/my-rag-store-123`"""

  config: Optional[UploadToRagStoreConfigDict]
  """Used to override the default configuration."""


_UploadToRagStoreParametersOrDict = Union[
    _UploadToRagStoreParameters, _UploadToRagStoreParametersDict
]


class UploadToRagStoreResponse(_common.BaseModel):
  """Response for the upload to rag store method."""
  ...
  parent: Optional[str] = Field(
      default=None,
      description="""The name of the RagStore containing Documents.""",
  )
  document_name: Optional[str] = Field(
...
class UploadToRagStoreResponseDict(TypedDict, total=False):
  """Response for the upload to rag store method."""
  ...
  parent: Optional[str]
  """The name of the RagStore containing Documents."""

  document_name: Optional[str]
  """The identifier for the Document imported."""


UploadToRagStoreResponseOrDict = Union[
    UploadToRagStoreResponse, UploadToRagStoreResponseDict
]


class UploadToRagStoreOperation(_common.BaseModel, Operation):
  """Long-running operation for ImportFile."""

  response: Optional[UploadToRagStoreResponse] = Field(
      default=None,
      description="""The result of the UploadToRagStore operation, available when the operation is done.""",
  )
  ...
  def from_api_response(
    ...
    """Instantiates a UploadToRagStoreOperation from an API response."""

    response_dict = _UploadToRagStoreOperation_from_mldev(api_response)
    return cls._from_response(response=response_dict, kwargs={})

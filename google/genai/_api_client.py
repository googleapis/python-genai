...
def patch_http_options(options: HttpOptions, patch_options: HttpOptions) -> HttpOptions:
...
class BaseApiClient:
    ...
    def _ensure_httpx_ssl_ctx(
        ...
        ctx = (
            args.get(verify)
            if args
            else None or async_args.get(verify) if async_args else None
        )
    ...
    def _use_aiohttp(self) -> bool:
        ...
        return (
            has_aiohttp
            and (self._http_options.async_client_args or {}).get("transport") is None
    ...
    def _build_request(
        ...
        url = base_url
        if not self.custom_base_url or (self.project and self.location) or self.api_key:
            if patched_http_options.base_url_resource_scope == ResourceScope.COLLECTION:
        ...
        populate_server_timeout_header(patched_http_options.headers, timeout_in_seconds)
        return HttpRequest(
            method=http_method,
            url=url,
...
def get_token_from_credentials(
    client: "BaseApiClient", credentials: google.auth.credentials.Credentials
) -> str:

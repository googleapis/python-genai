...
class Part(_common.BaseModel):
    ...
    def from_uri(
        ...
        Args:
          ...
          mime_type (str): mime_type: The MIME type of the image.
              If not provided, the MIME type will be automatically determined.
        """
        if mime_type is None:
            ...
            if not mime_type:
                raise ValueError(
                    f"Failed to determine mime type for file: {file_uri}"
                )
...
class JSONSchema(pydantic.BaseModel):
    ...
    The difference between this class and the Schema class is that this class is compatible with OpenAPI 3.1 schema objects. And the Schema class is used to make API call to Gemini model.
    """
        ...
        def convert_schema(
            ...
            for field_name, field_value in schema_dict.items():
                if field_value is None:
                    ...
                    schema_field_value: "JSONSchema" = convert_schema(
                        field_value
                    )
        ...
        *,
        ...
        def normalize_json_schema_type(
            json_schema_type: Optional[
                Union[
                    JSONSchemaType, Sequence[JSONSchemaType], str, Sequence[str]
                ]
            ],
        ) -> tuple[list[str], bool]:
...
class FunctionDeclaration(_common.BaseModel):
    ...
    def from_callable_with_api_option(
        ...
        *,
        ...
        Supported API option is 'VERTEX_AI' or 'GEMINI_API'.
        If api_option is unset, it will default to 'GEMINI_API'.
        If unsupported api_option is provided, it will raise ValueError.
        """
        supported_api_options = ["VERTEX_AI", "GEMINI_API"]
        if api_option not in supported_api_options:
            raise ValueError(
                f"Unsupported api_option value: {api_option}. Supported api_option value is one of:"
                f" {supported_api_options}."
            )
        ...
        for name, param in inspect.signature(callable).parameters.items():
            if param.kind in (
                ...
                if isinstance(param.annotation, str):
                    param = param.replace(
                        annotation=annotation_under_future[name]
                    )
        ...
        declaration = FunctionDeclaration(
            ...
            description=(
                inspect.cleandoc(callable.__doc__)
                if callable.__doc__
...
if _is_mcp_imported:
    ToolUnion = Union[
        Tool, Callable[..., Any], mcp_types.Tool, McpClientSession
    ]
    ToolUnionDict = Union[
...
class GenerationConfigRoutingConfig(_common.BaseModel):
    ...
    manual_mode: Optional[GenerationConfigRoutingConfigManualRoutingMode] = (
        Field(default=None, description="""Manual routing.""")
    )
    ...
    def _convert_literal_to_enum(cls, value: Any) -> Union[Any, EnumMeta]:
        if typing.get_origin(value) is typing.Literal:
            ...
            if not all(isinstance(arg, str) for arg in enum_vals):
                ...
                raise ValueError(
                    f"Literal type {value} must be a list of strings."
                )
...
class GenerateContentResponse(_common.BaseModel):
    ...
    def _get_text(self, warn_property: str = "text") -> Optional[str]:
        ...
        if len(self.candidates) > 1:
            logger.warning(
                f"there are {len(self.candidates)} candidates, returning {warn_property} result from"
                " the first candidate. Access response.candidates directly to get"
                " the result from other candidates."
            )
        text = ""
        ...
        if (
            ...
            try:
                ...
                if result_text is not None:
                    result.parsed = response_schema.model_validate_json(
                        result_text
                    )
            ...
            response_schema, builtin_types.GenericAlias
        ) or isinstance(response_schema, type):
...
class Image(_common.BaseModel):
    ...
    def from_file(
        ...
        Args:
            location: The local path or Google Cloud Storage URI from which to
                load the image.
...
class _EditImageParameters(_common.BaseModel):
    ...
    reference_images: Optional[list[_ReferenceImageAPI]] = Field(
        default=None,
        description="""The reference images for Imagen 3 editing.""",
    )
...
class _UpdateModelParameters(_common.BaseModel):
    ...
    model: Optional[str] = Field(default=None, description="""""")
    config: Optional[UpdateModelConfig] = Field(
        default=None, description=""""""
    )
...
class Video(_common.BaseModel):
    ...
    def __repr__(self) -> str:
        ...
        return f"Video(uri={self.uri}, video_bytes={video_bytes}, mime_type={self.mime_type})"
...
class DatasetStats(_common.BaseModel):
    ...
    user_message_per_example_distribution: Optional[DatasetDistribution] = (
        Field(
            default=None,
...
class SupervisedTuningDatasetDistributionDict(TypedDict, total=False):
    ...
    buckets: Optional[
        list[SupervisedTuningDatasetDistributionDatasetBucketDict]
    ]
    """Output only. Defines the histogram bucket."""
...
class JobError(_common.BaseModel):
    ...
    code: Optional[int] = Field(
        default=None, description="""The status code."""
    )
    message: Optional[str] = Field(
        default=None,
...
class ListBatchJobsResponse(_common.BaseModel):
    ...
    next_page_token: Optional[str] = Field(default=None, description="""""")
    batch_jobs: Optional[list[BatchJob]] = Field(
        default=None, description=""""""
    )
...
class RawReferenceImage(_common.BaseModel):
    ...
    def _validate_mask_image_config(self, values: Any) -> Any:
        if "reference_type" in values:
            raise ValueError(
                "Cannot set internal reference_type field directly."
            )

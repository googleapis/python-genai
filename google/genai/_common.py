...
def align_key_case(target_dict: dict[str, Any], update_dict: dict[str, Any]) -> dict[str, Any]:
  ...
  """
  aligned_update_dict = {}
  # Map: normalized_target_key -> original_target_key_casing
  ...
  for key, value in update_dict.items():
    ...
    if normalized_update_key in target_keys_map:
      aligned_key = target_keys_map[normalized_update_key]  # Use target's original casing
    else:
      aligned_key = key  # Keep original key if no case-insensitive match
...
def recursive_dict_update(
  ...
  """
  # SDK http request key case is unstable, so align the update_dict key case to target_dict to ensure correct dict update.

...
-   By default, use the following models when using `google-genai`:
    ...
    -   **Image edition Tasks:** `gemini-2.5-flash-preview-image`
...
Here's how to generate images using the Imagen models. Start with te fast model
...
and it is recommended to use chat mode. Configs are not supported in this model (except modality).
...
# Create the chat
chat = client.chats.create(model="gemini-2.5-flash-preview-image")
...
Usually start with the fast model since it usually give good enough results and
